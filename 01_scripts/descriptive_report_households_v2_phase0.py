"""
Household Descriptive Report Generator - V2 (Phase 0 Processed Data)
Generates comprehensive descriptive tables using cleaned Phase 0 household data

Improvements over V1:
- Uses Phase 0 processed/cleaned datasets
- Better data quality (missing values handled in Phase 0)
- Standardized variable coding
...
Author: Claude Code
Date: 2025-11-23
Version: 2.0 (Phase 0 Processed Data)
"""
...
# Paths - Updated to use Phase 0 processed data
DATA_PATH = Path("/Users/emersonrburke/Desktop/working/working_thesis_211125/WFL-Analysis/02_outputs/datasets/phase_0_household_processed.csv")
...
print("=" * 80)
print("HOUSEHOLD DESCRIPTIVE REPORT V2 - Using Phase 0 Processed Data")
print("=" * 80)
print("\nLoading Phase 0 processed household data...")
df = pd.read_csv(DATA_PATH)
print(f"Loaded {len(df)} households with {len(df.columns)} variables")
print(f"Data Quality: Phase 0 processed (cleaned, standardized)")
print("=" * 80 + "\n")
...
table_1a.to_csv(OUTPUT_DIR / "Table_1A_Demographics_Continuous_v2.csv", index=False)
...
# Neighborhood - standardize case variations
if 'neighborhood' in df.columns:
    # Standardize neighborhood names (title case)
    df['neighborhood_clean'] = df['neighborhood'].str.strip().str.title()
    neigh_counts = df['neighborhood_clean'].value_counts()
    neigh_pct = df['neighborhood_clean'].value_counts(normalize=True) * 100
    for category in neigh_counts.head(20).index:  # Top 20 to avoid cluttering
        categorical_stats.append({
...
table_1b.to_csv(OUTPUT_DIR / "Table_1B_Demographics_Categorical_v2.csv", index=False)
...
table_2a.to_csv(OUTPUT_DIR / "Table_2A_Neighborhood_Perceptions_v2.csv", index=False)
print("\n✓ Table 2A: Neighborhood Perceptions")
...
vendor_labels = {
    'time_001': 'Street Vendor',
    'time_002': 'Wet Market',
    'time_003': 'Supermarket',
    'time_004': 'Convenience Store',
    'time_005': 'Restaurant',
    'time_006': 'Food Truck',
    'time_007': 'Other'
}
...
for i, var in enumerate(time_vars, 1):
    if var in df.columns:
        label = vendor_labels.get(var, f'Vendor Type {i}')
        travel_stats['Vendor Type'].append(label)
...
table_2b.to_csv(OUTPUT_DIR / "Table_2B_Travel_Times_v2.csv", index=False)
print("\n✓ Table 2B: Travel Times to Markets")
...
if 'foodexpenditure' in df.columns:
    ...
    exp_stats.to_csv(OUTPUT_DIR / "Table_3A_Food_Expenditure_v2.csv", index=False)
    print("\n✓ Table 3A: Food Expenditure")
...
if 'foodsharing_activity' in df.columns:
    ...
    if sharing_cols:
        ...
        for col in sharing_cols:
            activity = col.split('/')[-1].replace('_', ' ').title()
        ...
        table_3b.to_csv(OUTPUT_DIR / "Table_3B_Food_Sharing_v2.csv", index=False)
        print("\n✓ Table 3B: Food Sharing Activities")
...
if foodgroup_cols:
    ...
    for col in foodgroup_cols:
        food_group = col.split('/')[-1].replace('_', ' ').title()
        count = df[col].sum() if df[col].dtype in [int, float] else df[col].value_counts().get(1, 0)
        pct = (count / len(df)) * 100
    ...
    table_4a.to_csv(OUTPUT_DIR / "Table_4A_Food_Groups_Consumed_v2.csv", index=False)
    print("\n✓ Table 4A: Food Groups Consumed (sorted by prevalence)")
    print(table_4a.to_string(index=False))

# Table 4B: Calculate HDDS
if foodgroup_cols:
    ...
    hdds_stats.to_csv(OUTPUT_DIR / "Table_4B_HDDS_Distribution_v2.csv", index=False)
...
if 'foodexpenditure' in df.columns:
    ...
    summary_data['Indicator'].append('Median Food Expenditure (VND)')
...
table_5_summary.to_csv(OUTPUT_DIR / "Table_5_Integrated_Summary_v2.csv", index=False)
...
print("\n" + "=" * 80)
print("HOUSEHOLD DESCRIPTIVE REPORT V2 COMPLETE (Phase 0 Data)")
print("=" * 80)
print(f"\nAll tables saved to: {OUTPUT_DIR}")
print("\nV2 Improvements:")
print("  ✓ Uses Phase 0 processed/cleaned data")
print("  ✓ Better data quality (missing values handled)")
print("  ✓ Standardized neighborhood names")
print("  ✓ Improved vendor type labels")
print("\nGenerated tables (v2):")
print("  - Table_1A_Demographics_Continuous_v2.csv")
print("  - Table_1B_Demographics_Categorical_v2.csv")
print("  - Table_2A_Neighborhood_Perceptions_v2.csv")
print("  - Table_2B_Travel_Times_v2.csv")
print("  - Table_3A_Food_Expenditure_v2.csv")
print("  - Table_3B_Food_Sharing_v2.csv")
print("  - Table_4A_Food_Groups_Consumed_v2.csv")
print("  - Table_4B_HDDS_Distribution_v2.csv")
print("  - Table_5_Integrated_Summary_v2.csv")
print("\n" + "=" * 80)
